...
public:
    ...
    void create_lifeline() {
        for (int i = 0; i < MAX_LIFE; ++i) {
            ...
            printf(R"({"_parent":"%s","visibility":"%s","name":"%s","_type":"UMLLifeline","isMultiInstance":false,"_id":"%s","represent":"represent"})""\n",
                   parent_id.c_str(), visibility.c_str(), lifeline_name.c_str(), lifeline_id.c_str());
        }
    }
    ...
    void create_message() {
        for (int i = 0; i < MAX_MEG; ++i) {
            ...
            if (source == nullptr || target == nullptr || (message_sort == "createMessage" && !is_normal)) {
                continue ;
            }
    ...
    void to_instr() {
        ...
        for (int i = 0; i < lifelines.size(); ++i) {
            ...
            printf("PTCP_CREATOR %s %s\n", interaction_name->c_str(), lifeline_name->c_str());
...
int main() {
    ...
    for (auto & collaboration : collaborations) {
