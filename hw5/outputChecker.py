...
def process(read, lineNum):
    ...
    if eles[0] == 'ARRIVE':
        ...
        if towers[index] != getTower(eles[1], lineNum):
            printError('ELEVATOR UNMATCH TOWER!', lineNum)
        if int(eles[2]) > MAX_FLOOR or int(eles[2]) < MIN_FLOOR:
            printError('ELEVATOR ON A NON-EXISTENT FLOOR!', lineNum)
        if floors[index] - int(eles[2]) != 1 and floors[index] - int(eles[2]) != -1:
            printError('ILLEGAL ELEVATOR MOVEMENT!', lineNum)
        floors[index] = int(eles[2])
        ...
        if towers[index] != getTower(eles[1], lineNum):
            printError('ELEVATOR UNMATCHED TOWER!', lineNum)
        ...
        if floors[index] != int(eles[3]):
            printError('ELEVATOR UNMATCHED FLOOR!', lineNum)
        if towers[index] != getTower(eles[2], lineNum):
            printError('ELEVATOR UNMATCHED TOWER!', lineNum)
