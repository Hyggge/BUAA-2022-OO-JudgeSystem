...
class Req:
    def __init__(self, req):
        ...
        self.id = int(eles[0])
        self.ft = getTower(eles[2], -1)
        self.ff = int(eles[3])
        self.tt = getTower(eles[5], -1)
        self.tf = int(eles[6])
        self.finish = False
    ...
    def getToFloor(self):
        return self.tf
    def move(self, toTower, toFloor, lineNum):
        self.ft = toTower
        self.ff = toFloor
        if self.tt == toTower and self.tf == toFloor:
            self.finish = True
    def isFinish(self):
        return self.finish
...
class Elevator:
    ...
    def checkOpen(self, eles, lineNum, opTime):
        if self.type == ELEVATOR_TOWER:
            if (self.arrive >> getTower(eles[1], lineNum)) & 1 != 1:
                printError('Elevator can\'t open door in this building!', lineNum)
        if self.state != STATE_CLOSE:
    ...
    def checkIn(self, eles, lineNum, opTime):
        ...
        if req == None:
            ...
            if req.getFromFloor() != int(eles[3]) or req.getFromTower() != getTower(eles[2], lineNum):
    ...
    def checkOut(self, eles, lineNum, opTime):
        ...
        if req == None:
            ...
            req.move(getTower(eles[2], lineNum), int(eles[3]), lineNum)
            if req.isFinish():
...
def initElevator():
    ...
    flag[0] = 1
    lastOpTime[0] = -1.0
    elevators[1] = Elevator('ADD-building-1-A-8-0.6')
    elevators[2] = Elevator('ADD-building-2-B-8-0.6')
    elevators[3] = Elevator('ADD-building-3-C-8-0.6')
    elevators[4] = Elevator('ADD-building-4-D-8-0.6')
    elevators[5] = Elevator('ADD-building-5-E-8-0.6')
    elevators[6] = Elevator('ADD-floor-6-1-8-0.6-31')
