...
EXTEND_NUM_MAX = 3
IMPLEMENT_NUM_MAX = 3
RULE_LIST = ["R001", "R002", "R003", "R004", "R005", "R007", "R008", "R009"]
...
exception_list = []
# exception_list = random.sample(["R001", "R002", "R003", "R004", "R005", "R007", "R008"], 1)
# exception_list = ["R003"]
...
class Tree :
    ...
    def add_relation(self, son_id, father_id) :
        if (son_id not in self.tree) :
            self.tree[son_id] = []
        if (father_id not in self.invert) :
            self.invert[father_id] = []
        self.tree[son_id].append(father_id)
        self.invert[father_id].append(son_id)


    def judge_circle(self, son_id, father_id) :
        self.ret = False
        self.dfs_check(father_id, son_id, father_id)
        # 0 means success while -1 means failure
        return self.ret
    
    def judge_dup(self, son_id, father_id) :
        self.ret = False
        self.up_dfs(son_id, father_id)
        return self.ret
    
    def up_dfs(self, src, dst) :
        self.down_dfs(src, dst)
        if (self.ret == True) :
            return
        if (dst in self.tree.keys()) :
            for i in self.tree[dst] :
                self.up_dfs(src, i)
        

    def down_dfs(self, src, dst) :
        self.dfs_check(src, dst, src)
        if (self.ret == True) :
            return 
        if (src in self.invert.keys()) :
            for i in self.invert[src] :
                self.down_dfs(i, dst)


    def dfs_check(self, src, dst, end) :    
        if (src == dst) :
            self.ret = True
            return     
        if (self.ret == True or src not in self.tree.keys()) :
            return
        for it in self.tree[src] :
            if (it != end) :
                self.dfs_check(it, dst, end)
...
trans_disjoint_set = DisjointSet()
...
class UMLModel :
    ...
    def __get_class_name(self) :
        if (switch() and "R001" in exception_list) :
            return get_empty_field()
...
class UMLClass :
    ...
    def __get_operation_name(self) :
        if (switch() and "R001" in exception_list) :
            return get_empty_field()
        # Guarantee a certain probability of the same name
        if (random.uniform(0, 1) < 0.3 and self.operation_name_no_set) :
    ...
    def get_generalization(self) :
        if (random.uniform(0, 1) > 0.7) :
            ...
            if (switch() and ("R003" in exception_list or "R004" in exception_list)) :
                if ("R003" in exception_list and "R004" in exception_list) :
                    self.ownedElements.append(new_generalization)
                    extend_tree.add_relation(source_id, target_id)
                elif ("R003" in exception_list) :
                    if (extend_tree.judge_circle(source_id, target_id) == True) :
                        self.ownedElements.append(new_generalization)
                        extend_tree.add_relation(source_id, target_id)
                        return
                    elif (extend_tree.judge_dup(source_id, target_id) == False) :
                        self.ownedElements.append(new_generalization)
                        extend_tree.add_relation(source_id, target_id)
                elif ("R004" in exception_list) :
                    if (extend_tree.judge_circle(source_id, target_id) == False) :
                        self.ownedElements.append(new_generalization)
                        extend_tree.add_relation(source_id, target_id)

            elif (exception_switch == False and 
                    extend_tree.judge_circle(source_id, target_id) == False and 
                    extend_tree.judge_dup(source_id, target_id) == False) :
                self.ownedElements.append(new_generalization)
                extend_tree.add_relation(source_id, target_id)
    ...
    def get_associations(self) :
        num = random.randint(0, ASSOCIATION_NUM_MAX)
        for _ in range(num) :
            self.ownedElements.append(UMLAssociation(self._id))
            ...
            if (switch() and ("R003" in exception_list or "R004" in exception_list)) :
                if ("R003" in exception_list and "R004" in exception_list) :
                    ...
                    if (extend_tree.judge_circle(source_id, target_id) == True) :
                        ...
                        extend_tree.add_relation(source_id, target_id)
                        return
                    if (extend_tree.judge_dup(source_id, target_id) == False) :
                        self.ownedElements.append(new_generalization)
                        extend_tree.add_relation(source_id, target_id)
                elif ("R004" in exception_list) :
                    if (extend_tree.judge_circle(source_id, target_id) == False) :
...
class UMLAttribute :
    ...
    def __init__(self, name, parent_id, type):
        ...
        self.name = name
        # visibility
        if (parent_id in interface_id_set) :
            if (switch() and "R005" in exception_list) : 
...
class UMLAssociation :
    ...
    def get_ends(self) :
        name = self.__get_attribute_name()
        ref_id = random.choice(list(class_id_set))
        self.end1 = UMLAssociationEnd(name, self._id, ref_id)
        name = self.__get_attribute_name()
        ref_id = random.choice(list(class_id_set))   
        self.end2 = UMLAssociationEnd(name, self._id, ref_id)
...
class UMLAssociationEnd :  
    ...
    def __init__(self, name, parent_id, reference) :
        self._id = get_id()
        lut[self._id] = self
        self._parent = {"$ref" : parent_id}
        self.name = name
...
class UMLRegion :
    ...
    def __get_trans_guard(self) :
        if (switch() and "R009" in exception_list and self.trans_guard_no_set) :
            if (random.randint(0, 1) == 1) :
                return "Guard" + str(random.choice(list(self.trans_guard_no_set)))
            else :
                return get_empty_field()
        guard_no = self.trans_cnt
        self.trans_guard_no_set.add(guard_no)
        return "Guard" + str(guard_no)


    def __create_new_transition(self, source, target) :
        guard = self.__get_trans_guard()
        new_transition = UMLTransition(self._id, source, target, guard)
    ...
    def get_states(self) :
        ...
        trans_disjoint_set.add(new_pseudo_state._id)
        ...
        for _ in range(num) :
            ...
            trans_disjoint_set.add(new_state._id)
        ...
        for i in range(final_state_num) :
            ...
            trans_disjoint_set.add(new_final_state._id)
    ...
    def get_transitions(self) :
        ...
        if (random.uniform(0, 1) < 0.2 and self.final_state_id_set) :
            ...
            trans_disjoint_set.union(source, target)
            trans_pair.add((source, target))
        
        # initial state to normal state
        ...
        for _ in range(num) :
            ...
            trans_disjoint_set.union(self.pseudo_state_id, target)
            init_dst_id.add(target)
            trans_pair.add((self.pseudo_state_id, target))

        # normal state to normal state
        ...
        if (random.uniform(0, 1) > 0.2 and self.final_state_id_set) :
            ...
            for _ in range(num) :
                ...
                for state_id in self.state_id_set :
                    if (flag == 0 and trans_disjoint_set.is_linked(self.pseudo_state_id, state_id)) :
                        self.__create_new_transition(state_id, target)
                        trans_disjoint_set.union(state_id, target)
                        trans_pair.add((state_id, target))
                        break
                    if (flag == 1 and not trans_disjoint_set.is_linked(self.pseudo_state_id, state_id)) :
                        self.__create_new_transition(state_id, target)
                        trans_disjoint_set.union(state_id, target)
                        trans_pair.add((state_id, target))
                        break
        
        # final state to other state if exception permitted
        if (switch() and "R008" in exception_list and self.final_state_id_set and self.state_id_set) :
            source = random.choice(list(self.final_state_id_set))
            target = random.choice(list(self.state_id_set))
            self.__create_new_transition(source, target)
            trans_disjoint_set.union(source, target)
            trans_pair.add((source, target))
            
        # duplicated transtion
        num = random.randint(0, len(trans_pair) >> 1)
        for _ in range(num) :
            source, target = random.choice(list(trans_pair))
            self.__create_new_transition(source, target)
...
class UMLTransition :
    ...
    def __init__(self, parent_id, source, target, guard) :
        self._id = get_id()
        lut[self._id] = self
...
def switch() :
    return exception_switch and random.randint(0, 1) == 1
...
def get_UML() :
    ...
    for class_id in class_id_set :
        ...
        if (switch() and "R002" in exception_list) :
            classs.get_associations()
...
def get_json() :
    ...
    cmd = "java -jar U4T3.jar dump -s \"input_UML.mdj\" -n Model -t UMLModel > input_json.txt"
    os.system(cmd)

    for no in Project.state_machine_no_set :
        cmd = "java -jar U4T3.jar dump -s \"input_UML.mdj\" -n StateMachine" + str(no) + " -t UMLStateMachine >> input_json.txt"
        os.system(cmd)
...
if __name__ == '__main__' : 
    ...
    if (prob < 0.3) :
        ...
        exception_switch = True
        exception_list = [random.choice(RULE_LIST)] 
    else :
        exception_switch = True
        exception_list = random.sample(RULE_LIST, len(RULE_LIST))
    
    # exception_switch = True
    # exception_list = [random.choice(["R003"])]

    # print(exception_switch)
    # print(exception_list)
